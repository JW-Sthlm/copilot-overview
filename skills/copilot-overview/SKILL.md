...
Inject both `ENV_DATA` and `PROJECT_DATA` into `template.html`. The template has a tab switcher that shows Environment by default; the Projects tab is wired automatically if `PROJECT_DATA.projects.length > 0`.

For **standalone projects-only** output (when the user asks only for projects): set `RENDER_MODE = "projects_only"` as a top-level const in the rendered HTML. The template hides the Environment tab in this mode.

### Refresh flows

- **"refresh project status"** or **"refresh all projects"** → run full Steps 1-9 with no cache reads (forced regen)
- **"refresh project X"** → run Steps 1-2 only for that project, re-synthesize only that entry, update cache, re-render
